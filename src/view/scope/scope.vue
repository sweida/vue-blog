...
<div>
  ...
  <div class="main-content scroll">
    ...
    <div class="room_table">
      <el-table v-if="roleList" :data="roleList" style="width: 100%" v-loading="loading" size="mini" element-loading-text="拼命加载中" :row-class-name="tableRowClassName" @selection-change="handleSelectionChange">
      ...
      <el-form ref="role" :rules="rules" :model="role" label-width="80px">
        ...
        <el-form-item label="权限" class="el-form-item-input">
          ...
          <div class="role-permission-panel">
            ...
            <el-tree v-loading="treeLoading" ref="menuTree" :data="menu" :props="defaultProps" show-checkbox node-key="id" default-expand-all :current-node-key="0" :highlight-current="true" :expand-on-click-node="false" :check-strictly="false" :render-content="renderContent"
              @check-change="permissionCheckChange" class="role-tree scroll">
            </el-tree>
          </div>
        </el-form-item>
      </el-form>
              ...
              if (node.authNodeList) {
                node.authNodeList.forEach((item) => {
                  if (this.role.menus.indexOf(item.id) == -1) {
                    item.checked = false;
                    let index = this.auths.indexOf(item.id);
